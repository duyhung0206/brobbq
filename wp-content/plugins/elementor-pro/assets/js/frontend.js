...
tableOfContent.addEventListener('mousedown', (e) => {
    isDown = true;
    startX = e.pageX - 200;
    scrollLeft = tableOfContent.scrollLeft;
    console.log(startX);
});
...
tableOfContent.addEventListener('mouseup', () => {
    isDown = false;
});

tableOfContent.addEventListener('mousemove', (e) => {
    if (!isDown) return;
    e.preventDefault();
    const x = e.pageX - 200;
    const walk = (x - startX) * 1;
    console.log(scrollLeft - walk);
    tableOfContent.scrollLeft = scrollLeft - walk;
});

$(window).scroll(function () {
    jQuery('.elementor-menu-anchor').forEach(function (element) {
        if (detectScrollToElement(element)) {
            jQuery('.table-of-content .btn-href-level-1 a[href$="' + element.id + '"]').addClass('active');
        } else {
            jQuery('.table-of-content .btn-href-level-1 a[href$="' + element.id + '"]').removeClass('active');
        }
    })
});
...
function detectScrollToElement(element) {
