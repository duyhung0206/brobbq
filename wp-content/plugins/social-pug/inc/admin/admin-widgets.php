...
class DPSP_Top_Shared_Posts extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Social Pug: Top Shared Posts', 'social-pug' ),
			array( 'description' => __( 'Display the most shared posts from any custom post type.', 'social-pug' ), )
		);
	...
	public function widget( $args, $instance ) {
		...
		$post_types  		 = ( isset( $instance['post_types'] ) && !empty( $instance['post_types'] ) ? $instance['post_types'] : array() );
		...
		$top_shared_posts = get_option( 'dpsp_top_shared_posts', array() );
		...
		if( !empty( $top_shared_posts ) && !empty( $post_types ) ) {
			...
			$top_shared_posts = get_posts( array( 'post_type' => $post_types, 'include' => $top_posts_ids ) );
